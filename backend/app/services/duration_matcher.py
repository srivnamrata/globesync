...
class DurationMatcher:
    """Google translation service with downstream spoken-duration measurement."""
    ...
    async def translate_with_duration_matching(
        ...
        """
        Translates text and measures its estimated speaking time against the source segment.
        """
        ...
        if provider != "google":
            raise ValueError("TRANSLATION_PROVIDER must be 'google'.")

        current_text = await google_translate_service.translate_text(
            source_text, source_language, target_language
        )
        current_est_ms = speech_rate_estimator.estimate_speech_duration_ms(
