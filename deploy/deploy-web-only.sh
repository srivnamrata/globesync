...
SKIP_ENABLE_APIS="${SKIP_ENABLE_APIS:-0}"

case "$DEPLOY_SCOPE" in
  all|api|web) ;;
  *)
    echo "ERROR: DEPLOY_SCOPE must be one of: all, api, web" >&2
    exit 1
    ;;
esac
...
EOF
  ...
  API_URL="$(gcloud run services describe "$API_SERVICE" --region="$REGION" --format='value(status.url)')"
  if [[ -z "$API_URL" ]]; then
    echo "ERROR: Could not resolve existing API URL for $API_SERVICE" >&2
    exit 1
  fi
  echo "==> Reusing existing API URL: $API_URL"
fi
